...
The Draw.io MCP server uses a hybrid communication approach:

- **Server to Browser Extension**: Server-Sent Events (SSE) via `GET /events` endpoint
  - Provides real-time event streaming from server to browser
  - Lightweight, unidirectional communication for server-initiated updates
  
- **Browser Extension to Server**: HTTP POST via `POST /message` endpoint
  - Handles client-initiated requests and responses
  - JSON payload for structured data exchange

- **MCP Client to Server**: Standard I/O (stdio) transport
  - Uses the MCP protocol's stdio transport for tool invocations
  - Ensures compatibility with all MCP-compliant clients

This architecture provides efficient bi-directional communication while maintaining protocol simplicity and broad compatibility.
